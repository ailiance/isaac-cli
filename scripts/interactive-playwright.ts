...
/**
 * Interactive Playwright launcher for the Dirac VS Code extension.
 *
 * Overview:
 *  - Starts the mock Dirac API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the Dirac extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the Dirac sidebar.
 ...
 *        npm run test:e2e:ui
 *
 *   3. VS Code will launch with the Dirac extension loaded and gRPC recording enabled.
 *
...
import { DiracApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with Dirac extension is now running!")
