...
/**
 * Simple Dirac gRPC Server
 *
 * This script provides a minimal way to run the Dirac core gRPC service
 ...
 *   2. DiracApiServerMock (mock implementation of the Dirac API)
 ...
 *   DIRAC_DIST_DIR - Override distribution directory (default: PROJECT_ROOT/dist-standalone)
 *   DIRAC_CORE_FILE - Override core file name (default: dirac-core.js)
...
import { DiracApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const distDir = process.env.DIRAC_DIST_DIR || path.join(projectRoot, "dist-standalone")
const diracCoreFile = process.env.DIRAC_CORE_FILE || "dirac-core.js"
const coreFile = path.join(distDir, diracCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple Dirac gRPC Server...")
	...
	if (!fs.existsSync(coreFile)) {
		...
		console.error("  DIRAC_DIST_DIR - Override distribution directory")
		console.error("  DIRAC_CORE_FILE - Override core file name")
		console.error("")
	...
	try {
		...
		console.log("Dirac API Server started in-process")
	} catch (error) {
		console.error("Failed to start Dirac API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const diracTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "dirac-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("npx", ["tsx", path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: diracTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", path.join(distDir, "dirac-core.js")]

	const spawnArgs = USE_C8 ? ["c8", "--report-dir", covDir, "node", ...baseArgs] : ["node", ...baseArgs]

	console.log(`Starting Dirac Core Service... (useC8=${USE_C8})`)
	...
	const shutdown = async () => {
		...
		await DiracApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(diracTestWorkspace, { recursive: true, force: true })
	...
	console.log(`Dirac gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple Dirac server:", err)
		process.exit(1)
	})
}
