...
async function createAgentForRepo(repoPath: string): Promise<{ agent: IsaacAgent; sessionId: string }> {
	const agent = new IsaacAgent({ cwd: repoPath })
...
function subscribeToReviewUpdates(agent: IsaacAgent, sessionId: string) {
...
describe("Isaac ACP review commands", () => {
