/**
 * IsaacAgent - Decoupled ACP Agent implementation for Isaac CLI.
 *
 * This class implements the ACP (Agent Client Protocol) Agent interface,
 * allowing Isaac to be used programmatically without stdio dependency.
...
import type { IsaacMessageChange } from "@core/task/message-state"
import type { ApiProvider } from "@shared/api"
import type { IsaacAsk, IsaacMessage as IsaacMessageType } from "@shared/ExtensionMessage"
import { CLI_ONLY_COMMANDS, VSCODE_ONLY_COMMANDS } from "@shared/slashCommands"
import { getProviderModelIdKey } from "@shared/storage/provider-keys"
import { IsaacEndpoint } from "@/config.js"
...
import { ExternalIsaacWebviewProvider } from "@/hosts/external/ExternalWebviewProvider.js"
...
import { AuthHandler } from "@/services/auth/AuthHandler.js"
...
import { IsaacSessionEmitter } from "./IsaacSessionEmitter.js"
...
import type { IsaacAcpSession, IsaacAgentOptions, PermissionHandler } from "./public-types.js"
...
/**
 * Isaac's implementation of the ACP Agent interface.
 *
 * This agent bridges the ACP protocol with Isaac's core Controller,
 ...
 * For stdio-based ACP communication, use the AcpAgent wrapper class.
 */
export class IsaacAgent implements acp.Agent {
	async shutdown() {
	...
	private readonly options: IsaacAgentOptions
	private readonly ctx: CliContextResult

	/** Map of active sessions by session ID */
	public readonly sessions: Map<string, IsaacAcpSession> = new Map()

	/** WeakMap to associate IsaacAcpSession with its Controller without exposing it to consumers */
	readonly #sessionControllers = new WeakMap<IsaacAcpSession, Controller>()
	...
	private readonly sessionEmitters: Map<string, IsaacSessionEmitter> = new Map()
	 ...
	 */
	emitterForSession(sessionId: string): IsaacSessionEmitter {
		let emitter = this.sessionEmitters.get(sessionId)
		if (!emitter) {
			emitter = new IsaacSessionEmitter()
	...
	async initialize(params: acp.InitializeRequest, connection?: acp.AgentSideConnection): Promise<acp.InitializeResponse> {
		...
		await IsaacEndpoint.initialize(this.ctx.EXTENSION_DIR)
	...
	initializeHostProvider(clientCapabilities?: acp.ClientCapabilities, connection?: acp.AgentSideConnection): void {
		...
		HostProvider.initialize(
			"cli",
			() => new ExternalIsaacWebviewProvider(this.ctx.extensionContext),
			() => {
	...
	async newSession(params: acp.NewSessionRequest): Promise<acp.NewSessionResponse> {
		...
		Logger.debug("[IsaacAgent] newSession called:", {
			sessionId,
			cwd: params.cwd,
		})
	...
	async loadSession(params: acp.LoadSessionRequest): Promise<acp.LoadSessionResponse> {
		...
		Logger.debug("[IsaacAgent] loadSession called:", { sessionId })
	...
	private async getSessionConfigOptions(session: IsaacAcpSession): Promise<acp.SessionConfigOption[]> {
		const stateManager = StateManager.get()
	...
	async unstable_setSessionModel(params: acp.SetSessionModelRequest): Promise<acp.SetSessionModelResponse> {
		...
		Logger.debug("[IsaacAgent] unstable_setSessionModel called:", {
	...
	async unstable_setSessionConfigOption(
		...
		Logger.debug("[IsaacAgent] unstable_setSessionConfigOption called:", {
			sessionId: params.sessionId,
	...
	private async applyProviderConfigOption(session: IsaacAcpSession, providerValue: string): Promise<void> {
	...
	private async applyModelConfigOption(session: IsaacAcpSession, modelValue: string): Promise<void> {
		const stateManager = StateManager.get()
	...
	private applyReasoningEffortConfigOption(session: IsaacAcpSession, effort: string): void {
	...
	private applyThinkingBudgetConfigOption(session: IsaacAcpSession, budgetValue: string): void {
	...
	private async applyProviderAndModel(session: IsaacAcpSession, provider: ApiProvider, modelId: string): Promise<void> {
		this.setModeScopedSessionState(session.mode, (mode) => {
			const providerKey = mode === "act" ? "actModeApiProvider" : "planModeApiProvider"
	...
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		Logger.debug("[IsaacAgent] prompt called:", {
			sessionId: params.sessionId,
		...
		try {
			...
			// try to continue the dead task and re-throw "Isaac instance aborted",
			...
			if (isLoadedSession && !hasActiveTask) {
				...
				Logger.debug("[IsaacAgent] Resuming loaded session:", params.sessionId)
				...
				Logger.debug("[IsaacAgent] Continuing existing task:", controller.task.taskId)
				...
				if (lastAskMessage) {
					...
					Logger.debug("[IsaacAgent] No pending ask found, starting new task")
					await controller.initTask(textContent, imageContent, fileResources)
				}
			} else {
				// Start new task
				Logger.debug("[IsaacAgent] Starting new task")
				await controller.initTask(textContent, imageContent, fileResources)
			}
			...
			for (const cleanup of cleanupFunctions) {
				try {
					...
					Logger.debug("[IsaacAgent] Error during cleanup:", error)
				}
			}
	...
	private async handleIsaacMessagesChanged(
		sessionId: string,
		sessionState: AcpSessionState,
		change: IsaacMessageChange,
		resolvePrompt: (response: acp.PromptResponse) => void,
		promptResolved: { value: boolean },
	): Promise<void> {
		Logger.debug("[IsaacAgent] handleIsaacMessagesChanged:", change)
		try {
			switch (change.type) {
				case "add":
	 ...
	 * 3. Responds to Isaac's ask based on the decision
	 *
	 * @param sessionId - The session ID
	 * @param sessionState - The session state
	 * @param message - The Isaac ask message
	 * @param permissionRequest - The permission request details from translateMessage
	 */
	private async handlePermissionRequest(
		sessionId: string,
		sessionState: AcpSessionState,
		message: IsaacMessageType,
		permissionRequest: Omit<acp.RequestPermissionRequest, "sessionId">,
	): Promise<void> {
		const session = this.sessions.get(sessionId)

		if (!session) {
			Logger.debug("[IsaacAgent] No session found for permission request")
			return
		}
		...
		if (!controller?.task) {
			Logger.debug("[IsaacAgent] No active task for permission request")
			return
		}

		const askType = message.ask as IsaacAsk

		try {
			// Request permission from the client
			const response = await this.requestPermission(sessionId, permissionRequest.toolCall, permissionRequest.options)

			Logger.debug("[IsaacAgent] Permission response received:", response.outcome)
	 ...
	 *
	 * This method uses translateMessage to properly map IsaacMessages to ACP SessionUpdates,
	...
	private async processMessageWithDelta(
		...
		if (isTextStreamingMessage && message.text && !isWebSearchMarkerMessage) {
			...
			if (result.requiresPermission && result.permissionRequest && !message.partial) {
				...
				// This will request permission from the client and respond to Isaac
	...
	async cancel(params: acp.CancelNotification): Promise<void> {
		const session = this.sessions.get(params.sessionId)
		if (!session) {
			Logger.debug("[IsaacAgent] cancel called for non-existent session:", params.sessionId)
			return
		}
		const sessionState = this.sessionStates.get(params.sessionId)

		Logger.debug("[IsaacAgent] cancel called:", {
			sessionId: params.sessionId,
		...
		if (sessionState) {
			...
			if (controller?.task) {
				try {
					...
					Logger.debug("[IsaacAgent] Error cancelling task:", error)
				}
			}
		}
	}
	...
	async setSessionMode(params: acp.SetSessionModeRequest): Promise<acp.SetSessionModeResponse> {
		...
		Logger.debug("[IsaacAgent] setSessionMode called:", {
			sessionId: params.sessionId,
	...
	private async emitSessionUpdate(sessionId: string, update: acp.SessionUpdate): Promise<void> {
		...
		try {
			...
			Logger.debug("[IsaacAgent] Error emitting session update:", error)
	...
	private async sendAvailableCommands(sessionId: string, controller: Controller): Promise<void> {
		try {
			// Get all available commands from Isaac
			...
			Logger.debug("[IsaacAgent] Sent available commands:", {
				sessionId,
				commandCount: availableCommands.length,
				commands: availableCommands.map((c) => c.name),
			})
		} catch (error) {
			Logger.debug("[IsaacAgent] Error sending available commands:", error)
		}
	}
}
