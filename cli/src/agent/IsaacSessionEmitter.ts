...
import type { IsaacSessionEvents } from "./public-types.js"

/**
 * Type-safe EventEmitter for IsaacAgent session events.
 *
 ...
 * @example
 * ```typescript
 * const agent = new IsaacAgent({ version: "1.0.0" })
 ...
 * ```
 */
export class IsaacSessionEmitter {
	 ...
	 */
	on<K extends keyof IsaacSessionEvents>(event: K, listener: IsaacSessionEvents[K]): this {
	 ...
	 * @returns This emitter instance for chaining
	 */
	once<K extends keyof IsaacSessionEvents>(event: K, listener: IsaacSessionEvents[K]): this {
	 ...
	 * @returns This emitter instance for chaining
	 */
	off<K extends keyof IsaacSessionEvents>(event: K, listener: IsaacSessionEvents[K]): this {
	 ...
	 */
	emit<K extends keyof IsaacSessionEvents>(event: K, ...args: Parameters<IsaacSessionEvents[K]>): boolean {
	 ...
	 * @returns This emitter instance for chaining
	 */
	removeAllListeners<K extends keyof IsaacSessionEvents>(event?: K): this {
		if (event) {
	 ...
	 */
	listenerCount<K extends keyof IsaacSessionEvents>(event: K): number {
