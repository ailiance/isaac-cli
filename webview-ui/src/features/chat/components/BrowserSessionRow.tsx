...
import { BrowserSettingsMenu } from "@/shared/ui/BrowserSettingsMenu"
import CodeBlock, { CODE_BLOCK_BG_COLOR } from "@/shared/ui/CodeBlock"
import { ProgressIndicator } from "./ChatRow/ChatRowComponents"
...
const BrowserSessionRow = memo((props: BrowserSessionRowProps) => {
	...
	const { browserSettings } = useSettingsStore()
	...
	useEffect(() => {
		if (actionHeight === 0 || actionHeight === Number.POSITIVE_INFINITY) {
			return // Skip if height is 0 or Infinity
		}
						...
						return (
							<div style={chatRowContentContainerStyle}>
								<div className="flex items-center gap-2 mb-2 text-xs opacity-70">
									<span className="codicon codicon-browser size-2" />
									<span className="font-bold text-success text-xs">Action Result</span>
								</div>
								{message.text && (
									<div style={codeBlockContainerStyle}>
										<CodeBlock source={`${"```"}shell\n${message.text}\n${"```"}`} />
			...
			<div style={browserActionBoxContainerInnerStyle}>
				<div style={browseActionRowContainerStyle}>
					<span style={browseActionRowStyle}>
						<span className="codicon codicon-browser mr-2" style={{ fontSize: "12px" }} />
