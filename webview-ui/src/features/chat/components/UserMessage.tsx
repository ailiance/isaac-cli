...
const UserMessage: React.FC<UserMessageProps> = ({ text, images, files, messageTs, sendMessageFromChatRow }) => {
	...
	const { checkpointManagerErrorMessage } = useSettingsStore()
	...
	return (
		<div
			className="p-3 my-2 text-foreground rounded-lg bg-(--vscode-badge-background)/30 border border-(--vscode-badge-background)/50 ml-8"
			onClick={handleClick}
			style={{
				backgroundColor: isEditing ? "unset" : undefined,
				whiteSpace: "pre-line",
				wordWrap: "break-word",
			}}>
			{isEditing ? (
