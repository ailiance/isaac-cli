...
export const CommandOutputContent = memo(
	({
		...
		const outputRef = useAutoScroll({
			dependency: output,
			enabled: !isOutputFullyExpanded,
		})
...
export const CommandOutputRow = memo(
	({
		...
		// If we have multiCommandState, we should NOT render the legacy command block if it's empty
		const shouldShowLegacyCommand = command && !multiCommandState
		...
		const content = (
			<>
				...
				<div className="bg-code rounded-sm border border-editor-group-border transition-all duration-300">
					{shouldShowLegacyCommand && (
						<div className="bg-code flex items-center justify-between px-2 py-2.5 border-b border-editor-group-border rounded-sm rounded-b-none overflow-hidden">
							...
							<div className="flex items-center gap-2 shrink-0">
								<CopyButton className="opacity-60 hover:opacity-100" textToCopy={command} />
								{showCancelButton && (
									<Button
										onClick={(e) => {
											e.stopPropagation()
				...
				{requestsApproval && !isCommandPending && (
					<div className="flex items-center gap-2.5 p-2 text-[12px] text-editor-warning-foreground">
						...
						<span>The model has determined this command require explicit approval.</span>
					</div>
				)}
			</>
		)

		if (multiCommandState) {
			const requiresApproval = multiCommandState.commands.some((cmd: any) => cmd.requiresApproval)
			const multiContent = (
				<div className="flex flex-col gap-3 w-full">
					...
					<div className="flex flex-col gap-2">
						{multiCommandState.commands.map((cmd: any, idx: number) => (
							<div className="bg-code rounded-sm border border-editor-group-border overflow-hidden" key={idx}>
								<div className="flex items-center justify-between px-2 py-2 border-b border-editor-group-border bg-code-header">
									<div className="flex items-center gap-2 flex-1 min-w-0">
										<div
											className={cn("bg-description rounded-full w-2 h-2 shrink-0", {
												"bg-success animate-pulse": cmd.status === "running",
												"bg-editor-warning-foreground": cmd.status === "pending",
												"bg-success": cmd.status === "completed",
												"bg-error": cmd.status === "failed",
												"opacity-50": cmd.status === "skipped",
											})}
										/>
										<span className="text-description font-medium text-sm break-all whitespace-pre-wrap">
											{cmd.command}
										</span>
										<CopyButton className="opacity-60 hover:opacity-100" textToCopy={cmd.command} />
									</div>
									<span className="text-[10px] opacity-60 uppercase ml-2 shrink-0">{cmd.status}</span>
								</div>
								{cmd.output && (
									<CommandOutputContent
										isContainerExpanded={true}
					...
					{requiresApproval && (
						<div className="flex items-center gap-2.5 p-2 text-[12px] text-editor-warning-foreground">
							<i className="codicon codicon-warning" />
							<span>The model has determined these commands require explicit approval.</span>
						</div>
					)}
				</div>
			)
...
function getCommandStatusText(isExecuting: boolean, isPending: boolean, isCompleted: boolean): string {
	...
	// Fallback for command messages that are no longer the last message but aren't completed yet
	return CommandStatusMap.pending
}
