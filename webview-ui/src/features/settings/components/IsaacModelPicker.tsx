...
export interface IsaacModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
const IsaacModelPicker: React.FC<IsaacModelPickerProps> = ({ isPopup, currentMode, showProviderRouting }) => {
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		if (freeIsaacModelIdSet.has(normalizeModelId(selected.selectedModelId))) {
			return {
				...selected,
				selectedModelInfo: {
		...
		return selected
	}, [apiConfiguration, currentMode, freeIsaacModelIdSet])
		...
		<div style={{ width: "100%", paddingBottom: 2 }}>
			...
			{hasInfo ? (
				...
				<p
					...
					The extension automatically fetches the latest Isaac model list.
				</p>
			)}
		</div>
	)
...
export default IsaacModelPicker
