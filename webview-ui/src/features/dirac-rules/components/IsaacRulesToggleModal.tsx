...
import { isMacOSOrLinux } from "@/shared/lib/platformUtils"
import PopupModalContainer from "@/shared/ui/PopupModalContainer"
...
const IsaacRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setGlobalIsaacRulesToggles,
		setLocalIsaacRulesToggles,
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleIsaacRule(
			ToggleIsaacRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Isaac rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: IsaacRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Isaac Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Isaac Rules & Workflows" : "Show Isaac Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-2 pt-0">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Isaac with system-level guidance. Think of them as a persistent way
								...
								<p>
									Workflows allow you to define a series of steps to guide Isaac through a repetitive set of
								...
								<p>
									Skills are reusable instruction sets that Isaac can activate on-demand. When a task matches a
									skill's description, Isaac uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Isaac's execution lifecycle,
								...
								{hasRemoteRules && (
									<div className="mb-3">
										...
										<div className="flex flex-col gap-0">
											{remoteGlobalRules.map((rule: any) => {
							...
							<>
								...
								{hasRemoteWorkflows && (
									<div className="mb-3">
										...
										<div className="flex flex-col gap-0">
											{remoteGlobalWorkflows.map((workflow: any) => {
												const enabled =
...
export default IsaacRulesToggleModal
