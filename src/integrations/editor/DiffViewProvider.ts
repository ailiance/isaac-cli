...
export abstract class DiffViewProvider {
	...
	async saveChanges(options?: { skipDiagnostics?: boolean }): Promise<{
		...
		const newProblems = options?.skipDiagnostics ? "" : await this.getNewDiagnosticProblems(normalizedPostSaveContent)
	...
	/**
	 * Applies the provided content and saves the file without any UI interaction.
	 * This is used for fast batch processing.
	 */
