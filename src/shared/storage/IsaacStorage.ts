...
export interface IsaacStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: IsaacStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and IsaacSyncStorage both satisfy this interface.
 */
export interface IsaacMemento {
 ...
 */
export abstract class IsaacStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "IsaacStorage"
	/**
 ...
 * Unlike IsaacStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: IsaacStorageChangeEvent) => void

export abstract class IsaacSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of IsaacStorage using a Map.
 */
export class IsaacInMemoryStorage extends IsaacStorage {
	/**
