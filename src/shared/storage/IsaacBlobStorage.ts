...
import { IsaacStorage } from "./IsaacStorage"
...
/**
 * S3/R2 blob storage implementation of IsaacStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class IsaacBlobStorage extends IsaacStorage {
	override name = "IsaacBlobStorage"

	private static store: IsaacBlobStorage | null = null
	static get instance(): IsaacBlobStorage {
		if (!IsaacBlobStorage.store) {
			IsaacBlobStorage.store = new IsaacBlobStorage()
		}
		return IsaacBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!IsaacBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[IsaacBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[IsaacBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = IsaacBlobStorage.instance
