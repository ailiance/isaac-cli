...
/**
 * Singleton manager for Isaac's temporary files.
 */
class IsaacTempManagerImpl {
	...
	/**
	 * Clean up old Isaac temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Isaac temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Isaac temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Isaac temp cleanup", error)
		}
...
export const IsaacTempManager = new IsaacTempManagerImpl()
