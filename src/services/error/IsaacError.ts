...
export class IsaacError extends Error {
	readonly title = "IsaacError"
	...
	/**
	 * Parses a stringified error into a IsaacError instance.
	 */
	static parse(errorStr?: string, modelId?: string): IsaacError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return IsaacError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a IsaacError instance.
	 * Always returns a IsaacError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): IsaacError {
		try {
			// If already a IsaacError, return it directly to prevent infinite recursion
			if (error instanceof IsaacError) {
				return error
			}
			return new IsaacError(JSON.parse(error), modelId, providerId)
		} catch {
			return new IsaacError(error, modelId, providerId)
		}
	}

	public isErrorType(type: IsaacErrorType): boolean {
		return IsaacError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: IsaacError): IsaacErrorType | undefined {
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return IsaacErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = IsaacErrorType.Network
	}
}
