...
/**
 * Isaac implementation of the telemetry provider interface
 * Handles Isaac-specific analytics tracking
 */
export class IsaacTelemetryProvider implements ITelemetryProvider {
	private telemetrySettings: TelemetrySettings
	private optInCache: boolean

	readonly name = "IsaacTelemetryProvider"
	...
	public async initialize(): Promise<IsaacTelemetryProvider> {
	...
	public log(event: string, properties?: TelemetryProperties): void {
		...
		// Log to Isaac telemetry endpoint
		this.captureToIsaac(event, properties)
	}

	public logRequired(event: string, properties?: TelemetryProperties): void {
		this.captureToIsaac(event, {
			...properties,
	...
	private async captureToIsaac(event: string, properties?: TelemetryProperties) {
		...
		try {
			...
			Logger.error(`Failed to send telemetry to Isaac: ${event}`, error)
		}
	}
