...
export class TelemetryService {
	...
	public capture(event: { event: string; properties?: TelemetryProperties }): void {
		const propertiesWithMetadata = this.getStandardAttributes(event.properties)
	...
	public captureRequired(event: string, properties?: TelemetryProperties): void {
		const propertiesWithMetadata = this.getStandardAttributes(properties)
	 ...
	 */
	public identifyAccount(userInfo: any) {
		...
		// Notify all providers of the user identification
		this.providers.forEach((provider) => {
			try {
				provider.identifyUser(userInfo, this.getStandardAttributes())
			} catch (error) {
	...
	/**
	 * Records when a tool name parsed from a model response is rejected
	 * by runtime validation (e.g. fictional `digikey:search` or
	 * `kicad.new_project`). Best-effort; safe to call from validators
	 * that must not throw.
	 * @param name The rejected tool name as emitted by the model
	 ...
	 * @param ulid Unique identifier for the task
	 * @param commandName The name of the command (e.g., "newtask", "reportbug", or custom workflow name)
	 * @param commandType Whether it's a built-in command, custom workflow
	 */
	public captureSlashCommandUsed(ulid: string, commandName: string, commandType: "builtin" | "workflow" | "skill") {
		this.capture({
	...
	/**
	 * Records when a file edit (write_to_file, replace_in_file) is accepted by the user
	...
	/**
	 * Records when a file edit (write_to_file, replace_in_file) is rejected by the user
