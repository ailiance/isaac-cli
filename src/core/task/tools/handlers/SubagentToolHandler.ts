...
function collectPrompts(block: ToolUse, configuredSubagentName?: string): string[] {
	if (configuredSubagentName) {
		const dynamicPrompt = (block.params.prompt || block.params.prompt_1 || "")?.trim()
...
export class UseSubagentsToolHandler implements IFullyManagedTool {
		...
		const prompts = configuredSubagentName
			? [
					uiHelpers
						.removeClosingTag(
							block,
							block.params.prompt ? "prompt" : "prompt_1",
							(block.params.prompt || block.params.prompt_1 || "")?.trim(),
						)
						?.trim(),
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		await config.callbacks.removeLastPartialMessageIfExistsWithType("say", "use_subagents")
		await config.callbacks.removeLastPartialMessageIfExistsWithType("ask", "use_subagents")
