...
export class ContextLoader {
			...
			for (const relPath of filePaths) {
				...
				try {
					...
					const skeleton = await ASTAnchorBridge.getFileSkeleton(
						absolutePath,
						this.dependencies.isaacIgnoreController,
	...
	async loadContext(
		...
		const parseTextBlock = async (text: string): Promise<string> => {
			const {
				enrichedText,
				needsIsaacrulesFileCheck: needsCheck,
		...
		// Check isaacrulesData if needed
		const isaacrulesError = needsIsaacrulesFileCheck
			? await ensureLocalIsaacDirExists(this.dependencies.cwd, GlobalFileNames.isaacRules)
			: false

		return [processedUserContent, environmentDetails, isaacrulesError, availableSkills, isDirectResponse, directResponseText]
	}
