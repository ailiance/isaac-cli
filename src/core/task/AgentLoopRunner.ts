...
export class AgentLoopRunner {
	...
	/**
	 * Execute ReAct iterations: stream an API request, process chunks, handle
	 * tool calls, then continue with the tool results until the loop ends.
	 *
	 * The continuation that previously recursed (`makeRequest → makeRequest`
	 * with the tool results) is now expressed as a single `while` loop: the
	 * recursive tail call was a pure tail call (its return value was returned
	 * verbatim), so iterating with the carried-over state is behaviour-
	 * identical while removing the unbounded recursion depth. State carried
	 * between turns lives on `this.taskState` (metrics, ranges, abort flags),
	 * so only `userContent`/`includeFileDetails` need to be threaded here.
	 */
	async makeRequest(userContent: IsaacContent[], includeFileDetails = false): Promise<boolean> {
		// biome-ignore lint/correctness/noConstantCondition: loop exits via return/throw/continue
		while (true) {
			...
			try {
					...
					while (true) {
						...
						switch (chunk.type) {
							...
							case "tool_calls": {
								...
								await this.ctx.processNativeToolCalls(
									assistantTextOnly,
				...
				if (assistantHasContent) {
					...
					// Continuation: re-enter the loop with the tool results as the
					// next user content (previously a recursive makeRequest call).
					// includeFileDetails was omitted on the recursive call, so it
