...
/**
 * The raw model information returned by the Isaac API to list models
 */
interface IsaacRawModelInfo {
	id: string
	name: string
	...
	tiers?: any[] | null
	supported_parameters?: IsaacSupportedParams[] | null
}
...
async function fetchRawIsaacModels(): Promise<IsaacRawModelInfo[]> {
	...
	Logger.log("Isaac models source: Isaac API")
	return response.data.data as IsaacRawModelInfo[]
}

/**
 * Core function: Refreshes the Isaac models and returns application types
...
export async function refreshIsaacModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	pendingRefresh = (async () => {
		try {
			return await fetchAndCacheIsaacModels()
		} finally {
...
async function fetchAndCacheIsaacModels(): Promise<Record<string, ModelInfo>> {
	...
	try {
		const rawModels = await fetchRawIsaacModels()
		...
		try {
			...
			Logger.error("Error reading Isaac models from cache:", cacheError)
		}
	}
...
/**
 * Read cached Isaac models from disk
...
export async function readIsaacModelsFromCache(): Promise<Record<string, ModelInfo> | undefined> {
	try {
		...
		Logger.error("Error reading Isaac models from cache:", error)
	}
	return undefined
}
