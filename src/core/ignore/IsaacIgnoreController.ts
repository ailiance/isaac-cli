...
export class IsaacIgnoreController {
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processIsaacIgnoreIncludes(content)
	 ...
	 */
	private async processIsaacIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[IsaacIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
