...
/**
 * Extracts potential symbol names from user text.
 * Heuristically identifies camelCase, PascalCase (multi-word), or snake_case identifiers.
 * Singular lowercase or single-capitalized words are ignored.
 */
export function extractSymbolLikeStrings(text: string): string[] {
	if (!text) return []

	// 0) Strip fenced code blocks to avoid extracting from pasted code.
	const withoutCodeFences = text.replace(/```[\s\S]*?```/g, " ")

	// 1) Remove URLs to avoid false positives.
	const withoutUrls = withoutCodeFences.replace(/\b\w+:\/\/[^\s]+/g, " ")

	// 2) Match potential identifiers (starts with letter or underscore, at least 3 chars)
	...
	while ((match = tokenRegex.exec(withoutUrls))) {
		...
		// Rules for "not a singular word":
		// - Contains an underscore (snake_case)
		// - Contains a transition from lowercase to uppercase (camelCase or PascalCase with >1 word)
		const hasUnderscore = candidate.includes("_")
		const hasInternalCaseChange = /[a-z][A-Z]/.test(candidate)

		if (hasUnderscore || hasInternalCaseChange) {
