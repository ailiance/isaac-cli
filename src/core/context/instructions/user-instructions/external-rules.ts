...
/**
 * Gather the formatted ISAAC.md project guide from the workspace root.
 * Always loaded when present (no toggle) — it is the project's own context file,
 * typically generated by the /init command.
 */
export const getLocalIsaacMdRules = async (cwd: string) => {
	const isaacMdFilePath = path.resolve(cwd, GlobalFileNames.isaacMdFile)

	if (await fileExistsAtPath(isaacMdFilePath)) {
		if (!(await isDirectory(isaacMdFilePath))) {
			try {
				const ruleFileContent = (await fs.readFile(isaacMdFilePath, "utf8")).trim()
				if (ruleFileContent) {
					return `# ISAAC.md\n\nThe following project guide is provided by the ISAAC.md file at the workspace root (${cwd}). It documents the project's structure, build/run/test commands, architecture, and conventions.\n\n${ruleFileContent}`
				}
			} catch {
				Logger.error(`Failed to read ISAAC.md file at ${isaacMdFilePath}`)
			}
		}
	}
